...
```bash
# Системные зависимости для работы с Access базами  
...
# Или раздельно в 2 терминалах:
npm run start:backend  # API сервер на порту 8000
npm run start:frontend # React приложение на порту 5173
```

**Доступ**: http://localhost:5173 | **API**: http://localhost:8000

📖 **Подробная документация**: [README_STARTUP.md](./README_STARTUP.md)

## ✨ Возможности

- 📊 **Загрузка Excel и Access файлов** (.xlsx, .xls, .mdb, .accdb)
...
- **Flask** - Python API для работы с MDB файлами
- **XLSX** - работа с Excel файлами
- **mdb-tools** - парсинг Access баз данных
...
mdb_server.py           # Flask API для MDB файлов
```

## 📋 Основные функции

### 1. Загрузка файлов
- Поддержка Excel (.xlsx, .xls) и Access (.mdb, .accdb)
- Многостраничные файлы
- Автоматическое определение полей
- Настройка алиасов для листов/таблиц
...
VITE_ALLOWED_FILE_TYPES=".xlsx,.xls,.mdb,.accdb"
...
python mdb_server.py  # смотрите вывод в терминале
...
### Решение проблем:
- **"mdb-tools не установлен"**: `brew install mdb-tools`
- **"ECONNREFUSED localhost:8000"**: запустите `python mdb_server.py`
- **"Buffer is not defined"**: используйте серверный API (не клиентские библиотеки)
- **"Port 5173 is in use"**: Vite автоматически использует следующий порт (5174, 5175...)
...
2. Убедитесь что все зависимости установлены (`brew install mdb-tools`, `pip install flask flask-cors`)
3. Перезапустите сервера (`npm run start:all`)
4. Очистите кэш браузера

---

Сделано с ❤️ для упрощения работы с данными УЭЦН 🛢️
